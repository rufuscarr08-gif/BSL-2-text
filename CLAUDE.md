...
Use this file as the primary Claude Code guidance for this project. Codex uses `AGENTS.md`; keep the two files aligned when workflow rules change.
...
Claude may write concise markdown notes into the vault when:
...
When Claude writes a vault note, mark it clearly as Claude-authored using YAML frontmatter:
...
When appending to an existing note that may have Codex-authored content, add a section headed:

```md
## Claude Update - YYYY-MM-DD
```

Include `origin: claude` in frontmatter if the whole note is newly created by Claude. Do not remove or rewrite Codex-specific metadata unless the user asks.
...
- Never overwrite user, Claude, Codex, or vault content unrelated to the current task.
